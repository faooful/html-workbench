...
HTML Workbench is a local-first Electron app for editing and previewing HTML files side by side. Choose a folder, browse the `.html` files in it, edit one in the built-in textarea, and preview the rendered document in an iframe.

## What It Does

- Lists `.html` files from a chosen folder, sorted by most recently modified.
- Restores the last watched folder, last opened file, and theme preference.
...
```text
.html-workbench.json      # Last watched folder path
